...
public class SudokuSolver implements ISudokuSolver {
	...
	public void setup(int size1) {
		...
		int vars = size*size*size*size;
		D = new ArrayList<ArrayList<Integer>>(vars);
		
		//Initialize each (domain) D[X]...		
		for (int i = 0; i < vars; i++){
			D.add(getSodukuDomain());
		}
		
	}


	private ArrayList<Integer> getSodukuDomain() {
	...
	private void setPuzzle(ArrayList<Integer> solution) {
		for(int i = 0; i < solution.size(); i++){
			int col = i/puzzle.length;
			int row = i%puzzle.length;
			setValue(col, row, solution.get(i)); 
		}
	}
		...
		public ArrayList<Integer> FC(ArrayList<Integer> asn) {
			...
			int x = getIndexOfFirstZero(asn);
			ArrayList<ArrayList<Integer>> oldDomain = cloneDomain(D);

			for(int i = 0; i < oldDomain.get(x).size(); i++){
				int v = oldDomain.get(x).get(i);
				//System.out.println("x: " + x + " v: " + v + " " + oldDomain.get(x));
				if (AC_FC(x,v)){
					asn.set(x, v);
			...
			return null;
			/*
			ArrayList<Integer> solution = asn;
			
			return solution;//null - failure
			*/
		}
	
		private ArrayList<ArrayList<Integer>> cloneDomain(ArrayList<ArrayList<Integer>> asn) {
			ArrayList<ArrayList<Integer>> clone = new ArrayList<ArrayList<Integer>>(asn.size());
			for(int o = 0; o < asn.size(); o++){
				ArrayList<Integer> domains = new ArrayList<Integer>(asn.get(o).size());
				for(int i = 0; i < asn.get(o).size(); i++){
					int val = asn.get(o).get(i);
					domains.add(val);
				}
				clone.add(domains);
			}
		...
		private int getIndexOfFirstZero(ArrayList<Integer> asn) {
		...
		private boolean isComplete(ArrayList<Integer> asn) {
			for(int i : asn){
				if (i == 0){
					return false;
				}
